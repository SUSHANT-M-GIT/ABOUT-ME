import React, { useRef } from 'react'
import { useInView } from '../hooks/useInView'
import { contact } from '../../data/portfolio'
import './Contact.css'

export default function Contact() {
  const sectionRef = useRef(null)
  const inView = useInView(sectionRef, { threshold: 0.15 })

  return (
    <section
      className={`contact section ${inView ? 'in-view' : ''}`}
      ref={sectionRef}
      aria-labelledby="contact-heading"
    >
      {/* Background doorway visual */}
      <div className="contact-bg" aria-hidden="true">
        <div className="doorway">
          <div className="doorway-inner">
            <div className="doorway-light" />
            {/* Floating dots */}
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className="doorway-particle" style={{
                '--dx': `${-50 + Math.random() * 100}%`,
                '--dy': `${-80 + Math.random() * 60}%`,
                '--delay': `${i * 0.3}s`,
              }} />
            ))}
          </div>
          <div className="doorway-frame doorway-frame-l" />
          <div className="doorway-frame doorway-frame-r" />
          <div className="doorway-frame doorway-frame-t" />
        </div>
      </div>

      <div className="section-label">
        <span className="section-label-line" />
        <span className="section-label-text">06 · CONTACT</span>
        <span className="section-label-line" />
      </div>

      <div className="contact-inner">
        <div className="contact-content">
          {/* Main heading */}
          <h2 className="contact-heading" id="contact-heading">
            {contact.heading}
          </h2>
          <p className="contact-sub">{contact.subheading}</p>

          {/* Primary CTA */}
          <a
            href={`mailto:${contact.email}`}
            className="contact-cta"
            aria-label={`Send email to Sushant at ${contact.email}`}
          >
            <span>LET'S CONNECT</span>
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
              <path d="M3 9h12M9 3l6 6-6 6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </a>

          {/* Contact details */}
          <div className="contact-details">
            <a href={`mailto:${contact.email}`} className="contact-detail">
              <div className="detail-icon-wrap">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <rect x="2" y="4" width="20" height="16" rx="2"/>
                  <path d="m2 7 10 7 10-7"/>
                </svg>
              </div>
              <div>
                <p className="detail-label">Email</p>
                <p className="detail-value">{contact.email}</p>
              </div>
            </a>

            <div className="contact-detail">
              <div className="detail-icon-wrap">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z"/>
                  <circle cx="12" cy="9" r="2.5"/>
                </svg>
              </div>
              <div>
                <p className="detail-label">Location</p>
                <p className="detail-value">{contact.location}</p>
              </div>
            </div>
          </div>

          {/* Social links */}
          <div className="contact-socials">
            <a
              href={contact.github}
              target="_blank"
              rel="noopener noreferrer"
              className="social-link"
              aria-label="Sushant's GitHub profile"
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 0C5.374 0 0 5.373 0 12c0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23A11.509 11.509 0 0 1 12 5.803c1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576C20.566 21.797 24 17.3 24 12c0-6.627-5.373-12-12-12z"/>
              </svg>
              <span>GitHub</span>
            </a>

            <a
              href={contact.linkedin}
              target="_blank"
              rel="noopener noreferrer"
              className="social-link"
              aria-label="Sushant's LinkedIn profile"
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
              </svg>
              <span>LinkedIn</span>
            </a>

            <a
              href={contact.portfolio}
              target="_blank"
              rel="noopener noreferrer"
              className="social-link"
              aria-label="Sushant's portfolio"
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                <circle cx="12" cy="12" r="10"/>
                <path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/>
              </svg>
              <span>Portfolio</span>
            </a>
          </div>
        </div>
      </div>
    </section>
  )
}
